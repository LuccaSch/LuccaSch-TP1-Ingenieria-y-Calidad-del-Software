...
function agregarFechaReserva() {
  ...
  if (existeSuperposicion(diaReservaDTO)) {
    mostrarMensajeError("Ya existe un dia de reserva que se superpone con el ingresado");
    return;
  }
...
function buscarDisponibilidad() {
  ...
  const periodo = obtenerValorRadioButtonSeleccionado();

  // Verificar si algún campo es null o está vacío
  if (!cantAlumnos || !idDocente || !idAsignatura || !nombreDocente || !nombreAsignatura || !periodo) {
  ...
  const diasReservaDTO = filas.map((fila,index) => {
...
function obtenerValorRadioButtonSeleccionado() {
